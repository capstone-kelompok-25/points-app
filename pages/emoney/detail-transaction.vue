    ...
    <v-card elevation="0" width="400" class="mx-auto" color="#F0F4F9">
      <v-card width="400" class="mx-auto">
        ...
        <v-card width="280" class="mx-auto" elevation="0">
          <v-card-title>{{ emoneyTransaction.an_rekening }}</v-card-title>
          <v-card-title class="blue--text">Transaksi Berlangsung</v-card-title>
          <v-card-text>
            <span>E-money - {{ emoneyTransaction.bank_provider }}</span
            ><br />
            <span>{{ emoneyTransaction.nomor }}</span
            ><br />
          </v-card-text>
          ...
          <v-card-text>
            <span
              >Poin anda:
              {{ parsingMoney(emoneyTransaction.poin_account) }}</span
            ><br />
            <span
              >Poin yang akan Ditukar:
              {{ parsingMoney(emoneyTransaction.poin_redeem) }}</span
            ><br />
            <span
              >Sisa POIN:
              {{
                parsingMoney(
                  emoneyTransaction.poin_account - emoneyTransaction.poin_redeem
                )
              }}</span
            >
          </v-card-text>
...
export default {
  ...
  data: () => ({
    items: [
      ...
      },
      {
        ...
        disabled: true,
        href: "/emoney/detail-transaction",
      },
    ],
  }),
  layout: "user",
  methods: {
    paymentPIN() {
      this.$router.push("/emoney/confirm");
    },
    parsingMoney(param) {
      const saldo = param;
  ...
  computed: {
    emoneyTransaction() {
      return this.$cookies.get("emoneyTransaction");
    },
  },
};
</script>
